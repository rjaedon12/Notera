...
export default function LearnPage({ params }: PageProps) {
  ...
  const [adaptiveMode, setAdaptiveMode] = useState(true) // SM-2 adaptive mode enabled by default
  ...
  const generateQuestions = useMemo(() => {
    ...
    const sortedCards = [...set.cards].sort((a, b) => {
      ...
      if (adaptiveMode) {
        // Adaptive mode: Prioritize cards due for review (SM-2)
        // Cards with no progress or overdue nextReviewAt come first
        const dueDateA = progressA?.nextReviewAt ? new Date(progressA.nextReviewAt) : new Date(0)
        const dueDateB = progressB?.nextReviewAt ? new Date(progressB.nextReviewAt) : new Date(0)
        
        // Cards due or overdue come first
        const isOverdueA = dueDateA <= now
        const isOverdueB = dueDateB <= now
        
        if (isOverdueA && !isOverdueB) return -1
        if (!isOverdueA && isOverdueB) return 1
        
        // Then sort by mastery level (lowest first)
        const masteryA = progressA?.masteryLevel ?? 0
        const masteryB = progressB?.masteryLevel ?? 0
        return masteryA - masteryB
      } else {
        // Standard mode: Just sort by mastery level (lowest first)
        const masteryA = progressA?.masteryLevel ?? 0
        const masteryB = progressB?.masteryLevel ?? 0
        return masteryA - masteryB
      }
